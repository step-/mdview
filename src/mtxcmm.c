...
{
    /*
    We add link definitions and zero-width heading links to the **markdown
    source** and utilize MD4C's `render_url_escaped` function to URI-encode the
    link destination. Since the link text has zero width, it remains hidden
    on the page and cannot be clicked. This effectively allows such a link
    to function as an in-page anchor, similar to how an HTML tag's id="ID"
    attribute provides a named anchor "ID" that can be reached via a web address
    ending with "#ID". Each heading link is inserted three times, following slug
    formats commonly generated by various document processing applications.

    We cannot utilize `g_uri_escape_string` to encode the TITLE because
    it may return a string that differs from the string returned by
    `render_url_escaped` for equal inputs, which would result in a dead
    link. Instead, to encode TITLE, we run the `mtx_cmm_mtx` parser twice,
    setting the output format `MTX_CMM_OUTPUT_BARE_INLINE`. The first run
